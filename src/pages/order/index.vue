<template>
  <view class="order-detail__content">
    ...
    <view class="order-detail__card order-detail__status">
      <view class="order-detail__status-title">待付款</view>
      <view class="steps">
        <view class="progress-item progress-item--prev">
          <view class="progress-item__progress">
            <text class="progress-item__title">提交订单</text>
            <text class="icon"></text>
          </view>
          <view class="progress-item__subtitle">{{ data.create_time }}</view>
        </view>
        <view class="progress-item progress-item--current">
          <view class="progress-item__progress">
            <text class="progress-item__title">付款</text>
            <text class="icon"></text>
          </view>
          <view class="progress-item__subtitle">{{ data.paid_time }}</view>
        </view>
        <view class="progress-item progress-item--next">
          <view class="progress-item__progress">
            <text class="progress-item__title">发货</text>
            <text class="icon"></text>
          </view>
          <view class="progress-item__subtitle">{{ data.send_time }}</view>
        </view>
        <view class="progress-item progress-item--next">
          <view class="progress-item__progress">
            <text class="progress-item__title">确认收货</text>
            <text class="icon"></text>
          </view>
          <view class="progress-item__subtitle">{{ data.deal_time }}</view>
        </view>
      </view>
    </view>
    <view class="order-detail__card order-detail__opt">
      <text class="order-detail__opt-no">订单号: {{ data.id }}</text>
      <template v-if="data.status === OrderStatus.TO_PAID">
        <text class="order-detail__opt-tip">待支付：</text>
        <text class="order-detail__price">
    ...
    <view class="order-detail__card order-detail__goods">
      <view class="main_title">
        <view class="title_icon">
          <svg t="1649938974154" class="x-icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg"
            p-id="143600">
            <path
      ...
      <view class="good_card" v-for="item in data.orderDetails" :key="item.id">
        <view class="card_goods">
          ...
          <view class="good_info">
            ...
            <view class="good_attr">{{ item.attr }}</view>
          </view>
        </view>
        <view class="card_price">
          <price-vue :price="[item.market_price]" :has-fix="true" :cur-font="16" :num-font="16"></price-vue>
        </view>
        <view class="card_count font-lg">x{{ item.quantity }}</view>
        <view class="card_status">
          <p class="ft-md">等待付款</p>
        </view>
      </view>
      <view class="order-detail__goods-footer">
        <view class="order-detail__goods-labels">
          <p>运费：</p>
          <p>需付款：</p>
        </view>
        <view class="order-detail__goods-prices">
          <text class="price">
            <price-vue :price="[]" :has-fix="true" :cur-font="14" :num-font="14"></price-vue>
          </text>
          <text class="price">
    ...
    <view class="order-detail__card order-detail__footer">
      <view class="order-detail order-detail__footer-address">
        <view class="order-detail__address-symbol">
          <text class="icon">
          ...
          <text class="order-detail__address-symbol-tip">收货信息：</text>
        </view>
        <view class="order-detail__address-info">
          <p class="order-detail__address">{{ data.receive_info }}</p>
          <button type="button" class="order-detail__change-btn button button--primary button--s button--plain">
            修改地址
          </button>
        </view>
      </view>
      <view class="order-detail__footer-title">
        <text class="icon">
          <svg t="1649939197191" class="x-icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg"
            p-id="155012">
            <path
